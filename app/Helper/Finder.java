...
public class Finder<T> {
    ...
    public Finder<T> desc(boolean d){
        this.descending = d;
        return this;
    }
    ...
    public Finder<T> update(String col, Object v){
    ...
    public Finder<T> setToken(JsonNode tokenNode) {
    ...
    public Optional<T> optional(Long id){
        return Optional.ofNullable(obj(id));
    }

    public T single() {
        return queryT().order(key()).setMaxRows(1).findOne();
    }
    ...
    public List<T> canAll(Optional<String> s) {
        this.page = s.isPresent() && BaseController.isNumeric(s.get()) ? Integer.parseInt(s.get()) : 0;
        return all(page);
    }

    public List<T> canAll(DynamicForm form) {

        Optional<String> search = form.field("search").value();

        Optional<String> value = form.field("page").value();

        return canSearch(search).canAll(value);
    }
    ...
    public T formData(FormFactory factory, Http.Request request) {
        Form<T> form = factory.form(type).bindFromRequest(request);

        return formData(form);
    }

    public T formData(Long id, FormFactory factory, Http.Request request) {
        _setId(id);
        return formData(factory,request);
    }
    ...
    private T formData(Form<T> form) {
        ...
        for (Field field : this.getFields()) {
            ...
            String value = null;
            Optional<String> vk = form.field(fieldName).value();
            if (vk.isPresent()) {
                value = vk.get();
            }
            ...
            try {
                if (attr) {
                    ...
                    Optional<String> optional = form.field(col).value();
                    ...
                    if( isDate ){
                        ...
                        field.set(t, date);
                        Logger.of("Lof").warn(date.toString());
    ...
    public Finder<T> canSearch(Optional<String> s) {
        s.ifPresent(this::isSearch);
        return this;
    }
    ...
    private boolean isEntity2(Field field) {
        return isEntity(field) && ! (field.isAnnotationPresent(DisableChain.class)  && field.getAnnotation(DisableChain.class).aClass() == field.getType());
    }
    ...
    private boolean isForSearch(Field field) {
        return !isEntity(field) && (field.isAnnotationPresent(JsonProperty.class) || field.isAnnotationPresent(FormProp.class) || (field.isAnnotationPresent(Formula.class) && (field.getType() == String.class)));
    }
    ...
    private void tableName(Field f, String s, String s2) {
        ...
        for (Field field : declaredFields) {
            ...
            if (attr) {
                ...
                boolean isEntity = isEntity2(field);
    ...
    private Query<T> setPagination(Query<T> query, int a) {
        this.counted = (query.findCount() - 1) / limit;
    ...
    private String key() {
        String s = orderKey != null ? orderKey : currentProperty;
        return s + (descending ? " desc" : " asc");
    }
    ...
    private void putData(Object method,T t,ArrayNode oNode){
    ...
    private ArrayNode getJsonFields(){
        ArrayNode array = Json.newArray();
        T bean = newBean();
        Object[] declaredMethods = this.list(getMethods(), getFields(), true);
        for (Object method : declaredMethods) {
            this.putData(method,bean,array);
        }
        return array;
    }

    public Finder<T> setPageTitle(String pageTitle){
    ...
    public JsonNode page(int a) {
        ...
        node.set("fields",getJsonFields());

        node.set("page", nodeList);
        node.put("title",pageTitle);
    ...
    private boolean isDismissed(String methodName, FormProp annotation) {
        ...
        boolean dismissed = annotation.dismissed();
        boolean inArray = inArray(methodName);


        return dismissed && !inArray || disable;
    }
    ...
    private void putAttr(ObjectNode node, FormProp annotation, String name, Object idColumn, boolean isTable, Class<?> type) {
        ...
        String formName = entity ? name + idColumn : f.equals(".id") ? name : f;
        ...
        if( entity ) node.put("select", true);
        ...
        for (Object object : objects) {
            ...
            if (object.getClass() == Field.class) {
                ...
                try {
                    field.setAccessible(true);
                    boolean aStatic = Modifier.isStatic(field.getModifiers());
                    Object dV =  field.get(newObjectT);
            ...
            if ( !isDismissed(name,annotation) ) {
                childNode.add(node);
            }
        }
    ...
    private void createMissingNode(Class<?> type, ObjectNode node, Object o, FormProp formProp) {
        ...
        if (annotation) {
            ...
            if (this.targetQueryId != null && this.targetNode != null && formProp.id().equals(this.targetQueryId)) {
                ...
                node.put("key", formProp.accessKey());
            }
        } else {
