    ...
    this.CreateForm = function (obj) {
        ...
        return {
            ...
            addObject: function (obj) {
                var s = this;
                obj.sort(function(v,x){
                    return v.order - x.order;
                });
                obj.forEach(function (v) {
                    var group = document.createElement("div");
                    group.className = "uk-form-row";
                    var label = document.createElement("label");
                    label.setAttribute("for",v.field);
                    var txt = "textarea";
                    var ix = v.type === txt ? txt : "input";
                    var input = document.createElement(ix);
                    input.autocomplete = "off";
                    input.className = v.type !== "checkbox" && v.type !== "radio" ? "md-input" : "";
                    ...
                    if( selectCondition ){
                        var ox = isCheck ? stackCheckBox(value,undefined,v.name,dValue) : stackSelect(value,undefined,v.name,dValue,v.label,undefined,true);
                    ...
                    var o = {
                        object: v,
                        escape: v.escape !== undefined && v.escape,
                        label: label,
                        field: input
                    };
...
function stackCheckBox(json, callback, name, dValue) {
    ...
    defaultValue.value = "";
    defaultValue.innerHTML = "-- Select from list --";
...
function textarea( t ) {
    ...
    var div = document.createElement("div");
    div.className = "parsley-row uk-margin-top relative";
    var label = document.createElement("label");
    label.innerHTML = "...";
...
function checkName(obj){
    ...
    ctrl.request({
        url:$(obj).attr("data-href")+"?code="+encodeURIComponent(obj.value),
        success:function (res) {
            if( res !== ""){
                $("#auto_data").val(res);
                $(".v_data").show();
